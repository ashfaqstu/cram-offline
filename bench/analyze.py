...
Reads  bench/results/sweep_<device>.csv  and  whisper_rtf.csv
...
def main():
    ...
    # ── Chart 4: Whisper RTF ────────────────────────────────────────────────
    wp = os.path.join(RES, "whisper_rtf.csv")
    if os.path.exists(wp):
        w = pd.read_csv(wp)
        w["label"] = w["model"].str.replace("ggml-", "").str.replace("-q5_1.bin", "") + "\nt=" + w["threads"].astype(str)
        fig, ax = plt.subplots(figsize=(7.2, 4.0))
        cols = [ACCENT if v < 0.7 else CRIT for v in w["rtf"]]
        bars = ax.bar(w["label"], w["rtf"], color=cols, width=.55)
        for bar, v in zip(bars, w["rtf"]):
            ax.text(bar.get_x() + bar.get_width() / 2, v + .02, f"{v:.2f}",
                    ha="center", color=FG, fontweight="bold")
        ax.axhline(1.0, color=CRIT, ls="--", lw=1.2)
        ax.text(0.02, 1.03, "1.0 = real time; above this it cannot keep up",
                color=CRIT, fontsize=9, transform=ax.get_yaxis_transform())
        ax.set_ylabel("real-time factor (lower is better)")
        ax.set_title("Whisper on the LITTLE cluster\nbase cannot keep up with speech; tiny can")
        ax.grid(axis="y", alpha=.3)
        save(fig, "chart_whisper_rtf.png")
        tables.append(("Whisper RTF (LITTLE cluster, 11.0 s clip)", w[["model", "threads", "total_ms", "rtf"]]))
